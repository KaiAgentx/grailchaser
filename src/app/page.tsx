  ...
  if (screen === "home") return (
    <Shell title="GrailChaser">
      <div style={{ paddingTop: 24 }}>
        ...
        <button onClick={() => { setCheckName(""); setCheckRaw(0); setCheckPsa10(0); setCheckPsa9(0); setCheckPsa8(0); setAskingPrice(0); setScanPreview(null); setScanResult(null); setLookupError(""); setScreen("cardCheck"); }} style={{ width: "100%", padding: "20px", background: "linear-gradient(135deg, " + green + "15, " + green + "08)", border: "1px solid " + green + "30", borderRadius: 16, cursor: "pointer", textAlign: "left", marginBottom: 12 }}>
    ...
    <Shell title="Check a Card" back={() => setScreen("home")}>
      <div style={{ paddingTop: 20 }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <input value={checkName} onChange={e => setCheckName(e.target.value)} placeholder="2020 Prizm Silver Justin Herbert RC" style={{ width: "100%", background: surface2, border: "1px solid " + border, borderRadius: 12, padding: "14px 16px", minHeight: 48, color: text, fontFamily: font, fontSize: 16, outline: "none", boxSizing: "border-box" }} />
        </div>
        <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ marginBottom: 14 }}>
        <button disabled={lookingUp || checkName.length <= 3} onClick={async () => { if (lookingUp || checkName.length <= 3) return; setLookingUp(true); setLookupError(""); setCheckRaw(0); setCheckPsa10(0); setCheckPsa9(0); setCheckPsa8(0); try { const playerName = scanResult?.player || checkName.replace(/^\d{4}\s+/, "").replace(/#\S+/g, "").replace(/\b(Prizm|Topps|Panini|Bowman|Donruss|Select|Optic|Chrome|Silver|Gold|Holo|Refractor|Base|RC)\b/gi, "").replace(/\s+/g, " ").trim(); const yearMatch = checkName.match(/\b(19|20)\d{2}\b/); const year = scanResult?.year || (yearMatch ? +yearMatch[0] : 0); const setName = scanResult?.set || checkName.replace(/\b\d{4}\b/g, "").replace(/#\S+/g, "").replace(new RegExp("\\b" + playerName.split(" ").join("\\b|\\b") + "\\b", "gi"), "").replace(/\b(RC|Base)\b/gi, "").replace(/\s+/g, " ").trim(); const priceRes = await fetch("/api/price", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ player: playerName, year, set: setName, card_number: "", sport: "" }) }); const priceData = await priceRes.json(); if (priceData.error) { setLookupError(priceData.error); } else if (priceData.prices) { const p = priceData.prices; if (p.psa10 || p.psa9 || p.psa8 || p.raw) { if (p.psa10) setCheckPsa10(p.psa10); if (p.psa9) setCheckPsa9(p.psa9); if (p.psa8) setCheckPsa8(p.psa8); const raw = p.raw || (p.psa9 ? +(p.psa9 * 0.6).toFixed(2) : 0) || (p.psa10 ? +(p.psa10 * 0.35).toFixed(2) : 0); if (raw) setCheckRaw(raw); if (p.raw) setCheckRaw(p.raw); } else { setLookupError("No pricing data found for this card"); } } } catch(e: any) { setLookupError("Lookup failed: " + e.message); } finally { setLookingUp(false); } }} style={{ width: "100%", padding: "14px 16px", minHeight: 48, background: cyan + "15", border: "1px solid " + cyan + "30", borderRadius: 12, color: cyan, fontFamily: font, fontSize: 15, fontWeight: 600, cursor: lookingUp ? "wait" : "pointer", marginBottom: 4, opacity: checkName.length > 3 ? 1 : 0.4, transition: "opacity 0.2s" }}>{lookingUp ? <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}><span style={{ display: "inline-block", width: 14, height: 14, border: "2px solid " + cyan + "40", borderTopColor: cyan, borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />Searching...</span> : "Look Up Prices"}</button>
        {lookupError && <div style={{ fontSize: 12, color: muted, marginBottom: 10, textAlign: "center" }}>{lookupError}</div>}
