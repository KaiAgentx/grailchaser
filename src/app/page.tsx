  ...
  if (screen === "cardCheck") return (
    <Shell title="Check a Card" back={() => setScreen("home")}>
      <div style={{ paddingTop: 20 }}>
        ...
        <div style={{ marginBottom: 14 }}>
        <button onClick={async () => { if (checkName.length > 3) { setCheckRaw(0); setCheckPsa10(0); setCheckPsa9(0); setCheckPsa8(0); try { const playerName = scanResult?.player || checkName.replace(/^\d{4}\s+/, "").replace(/#\S+/g, "").replace(/\b(Prizm|Topps|Panini|Bowman|Donruss|Select|Optic|Chrome|Silver|Gold|Holo|Refractor|Base|RC)\b/gi, "").replace(/\s+/g, " ").trim(); const yearMatch = checkName.match(/\b(19|20)\d{2}\b/); const year = scanResult?.year || (yearMatch ? +yearMatch[0] : 0); const priceRes = await fetch("/api/price", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ player: playerName, year, set: scanResult?.set || "", card_number: "", sport: "" }) }); const priceData = await priceRes.json(); if (priceData.prices && (priceData.prices.psa10 || priceData.prices.psa9 || priceData.prices.raw)) { const raw = priceData.prices.raw || (priceData.prices.psa9 ? +(priceData.prices.psa9 * 0.6).toFixed(2) : 0) || (priceData.prices.psa10 ? +(priceData.prices.psa10 * 0.35).toFixed(2) : 0); if (raw) setCheckRaw(raw); if (priceData.prices.raw) setCheckRaw(priceData.prices.raw); if (priceData.prices.psa10) setCheckPsa10(priceData.prices.psa10); if (priceData.prices.psa9) setCheckPsa9(priceData.prices.psa9); if (priceData.prices.psa8) setCheckPsa8(priceData.prices.psa8); } } catch(e) {} } }} style={{ width: "100%", padding: "12px", background: cyan + "15", border: "1px solid " + cyan + "30", borderRadius: 10, color: cyan, fontFamily: font, fontSize: 13, fontWeight: 600, cursor: "pointer", marginBottom: 14, opacity: checkName.length > 3 ? 1 : 0.4 }}>Look Up Prices</button>
